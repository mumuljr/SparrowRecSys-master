...
* TensorFlow 2.0+
* Keras (随TensorFlow继承)
* PySpark(用于大规模数据分析与样本构建)
* HTML + CSS + JavaScript
* jQuery 3.2.1

## 运行准备
进入\python_embedding，先后运行：convert_ml25m_to_sparrow.py、large_scale_embedding_trainer.py、simple_embedding_trainer.py、ncf_embedding_train.py、generate_ncf_scores.py

## 快速开始
将项目用IntelliJ打开后，找到`RecSysServer`，右键点选`Run`，然后在浏览器中输入`http://localhost:6010/`即可看到推荐系统的前端效果。

## 项目数据
项目数据分为两类，来源于开源电影数据集GroupLens Research[MovieLens](https://grouplens.org/datasets/movielens/)，一类是MovieLens标准数据集，约 116 万条用户–电影评分记录，用于基础推荐模型训练，系统功能验证与快速调试；另一类为MovieLens 25M 数据集，约 2500 万条评分记录覆盖更多用户、电影与长尾内容，用于大规模高质量推荐模型训练，生成 *_large.csv 数据文件，训练大数据版本 embedding 模型与 NCF 模型。
...
* NCF, Neural Collaborative Filtering(新增)
* Large-scale Models(新增)
