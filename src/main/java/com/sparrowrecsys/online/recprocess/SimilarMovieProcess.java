...
public class SimilarMovieProcess {
    ...
    public static List<Movie> retrievalCandidatesByEmbedding(Movie movie, int size){
        ...
        List<Movie> allCandidates = DataManager.getInstance().getMovies(Integer.MAX_VALUE, "rating");
    ...
    public static List<Movie> ranker(Movie movie, List<Movie> candidates, String model) {
        ...
        boolean useEmbedding = movie.getEmb() != null;

        for (Movie candidate : candidates) {
            double similarity;

            if (useEmbedding && candidate.getEmb() != null) {
    ...
    public static double calculateEmbSimilarScore(Movie movie, Movie candidate) {
        if (movie == null || candidate == null) return -1;
        if (movie.getEmb() == null || candidate.getEmb() == null) return -1;
